mutable struct Controller4 <: Controller
    control!::Function

    function Controller4(F,Q,R)
        new(control!)
    end
end

function control!(mechanism,controller::Controller4,k)

    return
end

function run4!(str)
    open("Files/E4.jl",create=true,write=true) do file
        write(file,"
        # Parameters
        ex = [1.0;0.0;0.0]
        ey = [0.0;1.0;0.0]
        m1 = 0.135
        m2 = 0.123
        l1 = 0.140
        l2 = 0.135
        L1 = 0.280
        L2 = 0.270
        M = 4.5

        cartshape = Box(0.1, 0.5, 0.1, M)
        pend1shape = Box(0.1, 0.1, L1, L1)
        pend2shape = Box(0.1, 0.1, L2, L2)

        p1 = [0.0;0.0;l1] # joint connection point
        p2 = [0.0;0.0;l2] # joint connection point

        # Desired orientation
        θ1 = 0
        θ2 = 0

        # Links
        origin = Origin{Float64}()
        cart = Body(cartshape)
        pend1 = Body(pend1shape)
        pend2 = Body(pend2shape)

        # Constraints
        joint1 = EqualityConstraint(Prismatic(origin, cart, ey))
        joint2 = EqualityConstraint(Revolute(cart, pend1, ex; p2 = -p1))
        joint3 = EqualityConstraint(Revolute(pend1, pend2, ex; p1 = p1, p2 = -p2))

        links = [cart;pend1;pend2]
        constraints = [joint1;joint2;joint3]
        shapes = [cartshape;pend1shape;pend2shape]


        mech = Mechanism(origin, links, constraints, shapes = shapes, Δt = 0.01)
        setPosition!(origin,cart,Δx = [0;0.0;0])
        setPosition!(cart,pend1,p2 = -p1, Δq = UnitQuaternion(RotX(0.01)))
        setPosition!(pend1,pend2,p1 = p1, p2 = -p2, Δq = UnitQuaternion(RotX(0.01)))


        ### Begin Student Input

        ")
        write(file, str)
        write(file, "
        
        ### End Student Input

        xd = [[[0;0;0.0]]; [p1]; [2*p1+p2]]

        Q = [diagm(ones(12))*1.0 for i=1:3]
        R = [ones(1,1)]

        lqr = LQR(mech, getid.(bodies), [getid(constraints[1])], Q, R, 10., xd=xd)


        # controller = Controller4(F,Q,R)

        steps = Base.OneTo(1000)
        storage = Storage{Float64}(steps,2)

        try
            # simulate!(mech,storage,controller,record = true)
            simulate!(mech,storage,lqr,record = true)
        catch
            @info \"Unstable behavior\"
        end

        visualize(mech,storage,shapes)")
    end

    include("Files/E4.jl")

    return
end